...
/**
 * The standard-tier (WebGL2/GLSL-ES-3.00) printer: reads a function's `glslStandard`/
 * `glslStandardHelper` fields, falling back to `glslBaseline`/`glslBaselineHelper` when a function
 * prints identically either way. Independent implementation of `printGLSLBaseline.Internal.ts`:
 * the baseline and standard tiers are fully separated outside the IR and the compilation core, so
 * there is no shared print core to keep in sync between the two.
 */
export function printGLSLStandard(
  expression: FXExpr,
  ...
  const print = (node: FXExpr): string => {
    switch (node.kind) {
      case "lit": {
        ...
        const [scalarValue] = node.values;
        if (scalarValue === undefined) {
          throw new Error("sparcoon IR: scalar literal has no value component");
        }
        return printLiteral(scalarValue);
      }
      case "ref":
      ...
      case "call": {
        ...
        const spelling = definition.glslStandard ?? definition.glslBaseline;
        const spellingHelper = definition.glslStandardHelper ?? definition.glslBaselineHelper;
        if (spellingHelper !== undefined) {
          helpers.set(definition.name, spellingHelper);
        }
        if (typeof spelling === "function") {
          return spelling(argumentStrings);
        }
        const functionName = typeof spelling === "string" ? spelling : definition.name;
